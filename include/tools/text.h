...
public:
	...
	virtual SelfDrawingImage Reaction(SelfDrawingImage image, const SDL::events::Event& evt)override
	{
		if(evt.Type()==SDL::events::Type::MouseButtonDown)
		{
			...
			{
                image.Image().Draw(font, text, SDL::Color(0,0,0), image.Positioning().ToObject(mouse.Position)-font.TextSize(text)/2);
                image.Image().Confirm();
                image.AreaChanged(SDL::Rect(image.Positioning().ToObject(mouse.Position)-font.TextSize(text)/2, font.TextSize(text)));
                active=false;
			}
		}
	...
	virtual void DrawOn(SelfDrawingImage& image, SDL::Renderer& w)override
	{
		...
		w.Draw(textimg, textimg.Size(), image.Positioning().ToScreen(SDL::Rect(textimg.Size()).WithCenter(image.Positioning().ToObject(SDL::Cursor::Position()))));
	}
